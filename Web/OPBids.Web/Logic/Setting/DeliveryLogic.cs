...
{
    public class DeliveryLogic : SettingLogicBase
    {
        IOwinContext _context;
        public DeliveryLogic(IOwinContext httpContext)
        {
            this._context = httpContext;
        }
        public override string HeaderView(string subMenuId)
        {
            return Constant.Setting.HeaderView.Delivery;
        }
        ...
        {
            return DownloadExcel<DeliveryVM>("Delivery", Constant.ServiceEnpoint.Settings.GetDelivery);
        }
        public override Tuple<string, string> DownloadCSV()
        {
            return DownloadCSV<DeliveryVM>("Delivery", Constant.ServiceEnpoint.Settings.GetDelivery);
        }
        public override string HTMLTable()
        {
            return HTMLTable<DeliveryVM>(Constant.ServiceEnpoint.Settings.GetDelivery);
        }
        public override IEnumerable<T> SearchData<T>(SettingVM setting)
        {
            var rslts = ProcessData<T>(setting, Constant.ServiceEnpoint.Settings.GetDelivery);
        ...
        {
            return PartialView(Constant.Setting.ResultView.Delivery,
                SearchData<DeliveryVM>(setting));
        }
        ...
        public override ActionResult Save(SettingVM setting)
        {
            ...
            var _delivery = setting.delivery;
            _delivery.updated_by = user_id;
            var curUrl = Constant.ServiceEnpoint.Settings.CreateDelivery;
            if (_delivery.id == 0)
            {
                _delivery.created_by = user_id;
            }
            else
            {
                curUrl = Constant.ServiceEnpoint.Settings.UpdateDelivery;
            }
            Result<IEnumerable<DeliveryVM>> _list;
            var apiManager = new ApiManager<Result<IEnumerable<DeliveryVM>>>();
            ...
            ViewBag.page_reset = false;
            return PartialView(Constant.Setting.ResultView.Delivery,
                _list.value == null ? new List<DeliveryVM>() : _list.value);
        }
        ...
        public override ActionResult StatusUpdate(SettingVM setting)
        {
            ...
            var apiManager = new ApiManager<Result<IEnumerable<DeliveryVM>>>();
            _list = apiManager.Invoke(ConfigManager.BaseServiceURL,
                Constant.ServiceEnpoint.Settings.StatusUpdateDelivery, setting);
            ViewBag.total_count = _list.value.Count();
            ViewBag.page_count = _list.page_count;
            ViewBag.page_reset = false;
            return PartialView(Constant.Setting.ResultView.Delivery,
