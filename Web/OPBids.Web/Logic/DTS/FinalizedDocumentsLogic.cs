...
{
    public class FinalizedDocumentsLogic : DTSLogicBase
    {
        public override ActionResult PartialView(DocumentsPayloadVM param)
        {
            return PartialView(Constant.DTS.PartialView.Finalized, new List<DocumentsVM>());
        }
        public override Result<IEnumerable<T>> Maintain<T>(DocumentsPayloadVM param)
        {
            return ProcessData<T>(param, Constant.ServiceEnpoint.DTS.MaintainFinalized);
        }
    }
}