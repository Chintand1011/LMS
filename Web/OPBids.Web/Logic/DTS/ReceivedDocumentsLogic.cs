...
{
    public class ReceivedDocumentsLogic : DTSLogicBase
    {
        public override ActionResult PartialView(DocumentsPayloadVM param)
        {
            return PartialView(Constant.DTS.PartialView.Received, new List<DocumentsVM>());
        }
        public override Result<IEnumerable<T>> Maintain<T>(DocumentsPayloadVM param)
        {
            return ProcessData<T>(param, Constant.ServiceEnpoint.DTS.MaintainReceived);
        }
    }
}