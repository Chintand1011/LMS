...
{
    public class RequestBarcodeLogic : DTSLogicBase
    {
        public override ActionResult PartialView(DocumentsPayloadVM param)
        {
            return PartialView(Constant.DTS.PartialView.RequestBarcode, new List<RequestBarcodeVM>());
        }
        public override Result<IEnumerable<T>> Maintain<T>(DocumentsPayloadVM param)
        {
            return ProcessData<T>(param, Constant.ServiceEnpoint.DTS.MaintainRequestBarcode);
        }
    }
}