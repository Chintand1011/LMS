...
{
    public class DocumentRoutesLogic : DTSCommonLogicBase
    {
        IOwinContext _context;
        public DocumentRoutesLogic(IOwinContext httpContext)
        {
        ...
        public override IEnumerable<T> Maintain<T>(DocumentsPayloadVM param)
        {
            ...
            param.documentRoute.updated_by = user_id;
            if (param.documentRoute.id == 0)
            {
                param.documentRoute.created_by = user_id;
            }
            if (param.documentRoutes != null)
            {
            ...
            _list = apiManager.Invoke(ConfigManager.BaseServiceURL, Constant.ServiceEnpoint.DTS.MaintainDocumentRoutes, param);
