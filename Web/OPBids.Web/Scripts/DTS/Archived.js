var ARCD = {
    ...
    GetParameters: function (cProc) {
        ...
        param.document.id = ARCD.BatchId;
    ...
    Initialize: function () {
        ARCD.DocData = (new Date().getTime()).toString();
        ARCD.AttData = [ARCD.DocData, "-1"].join('');
        ...
        $(".ion-search").click(function () {
            ARCD.MaintainData(CONST.transaction_type.search, null);
        });
    ...
    MaintainData: function (currentProcess, cStat) {
        var param = ARCD.GetParameters(currentProcess);
        if (cStat != null && cStat != "") {
        ...
        var postEvt = function () {
            if (currentProcess == CONST.transaction_type.statusUpdate) {
                ...
                Site.Dialogs.Alert(["Document with batch number ", ARCD.BatchId, " is successfully submitted for Archiving. Document can be viewed under @Records Section Menu once Archiving Request has been accepted"].join(''), "Got It!", null);
                return;
            }
            var data = $("body").data(ARCD.DocData);
            $("#grdLst tbody").html('');
            Site.FillPagingValues(data.page_count, false);
            $(".totalDocs").html(data.total_count);
            if (data.value != null && data.value != undefined && data.value != "") {
                $(data.value).each(function () {
                    ARCD.BatchId = this.id;
                ...
                $("#grdLst tbody tr .lnkArchive").click(function () {
                    ARCD.BatchId = BatchId = $(this).attr("itemid");
                    var yesEvt = function () {
                        var postEvt = function () {
                            ARCD.MaintainData(CONST.transaction_type.statusUpdate, CONST.record_status.archiving);
                        }
                        ARCD.MaintainLogs(CONST.transaction_type.save, $("#dialogInput").val(), postEvt);
                    }
                ...
                $("#grdLst tbody tr .lnkView").click(function () {
                    ARCD.PopUpModal($(this).attr("itemid"));
                });
        ...
        Site.PostData("/DTS/MaintainDocuments", postEvt, param, ARCD.DocData);
    },
    PopUpModal: function(itemId){
        ARCD.BatchId = itemId;
        DTS.GenerateRouteBar(ARCD.BatchId);
        var sender = $(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[sender_id]"].join('')).html();
        var recipient = $(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[receipient_id]"].join('')).html();
        var etd_to_recipient = $(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[etd_to_recipient]"].join('')).html();
        var document_type = $(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[document_type_id]"].join('')).html();
        ARCD.CurrentProcess = CONST.transaction_type.update;
        $('#dataEntryModal').modal('show');
        $(".mainCaption #curDateCaption").html([$(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[sent_date]"].join('')).html().split(' ')[0], ":"].join(''));
        ...
        $("#generalTab").click();
        $(".modal-title").html($(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td[document_code]"].join('')).html());
        $(".batchno").html(ARCD.BatchId);
        Site.DrawWaterMark("#canvasImage", $(["#grdLst tbody tr[itemid='", ARCD.BatchId, "'] td span.delivery_type_name"].join('')).html());
        ARCD.MaintainRoutes(CONST.transaction_type.search);
        ARCD.MaintainAttachments(CONST.transaction_type.search);
        ARCD.MaintainLogs(CONST.transaction_type.search);
    },
    MaintainRoutes: function (curProcess) {
        var postEvt = function () {
            var data = $("body").data(ARCD.DocData);
            if (ARCD.CurrentProcess == CONST.transaction_type.statusUpdate || ARCD.CurrentProcess == CONST.transaction_type.promote) {
                $(".ion-search").click();
            }
            else {
        ...
        var hasData = false;
        var param = ARCD.GetParameters(curProcess);
        param.documentRoute.process = curProcess;
        param.documentRoute.batch_id = ARCD.BatchId;
        if (CONST.transaction_type.save == curProcess) {
            $("#lstRoutes tbody  tr:not([rowcopy]").each(function () {
                ...
                paramRoutes.batch_id = ARCD.BatchId;
        ...
        if (hasData == true) {
            Site.PostData("/DTS/MaintainDocumentRoutes", postEvt, param, ARCD.DocData);
        }
    },
    MaintainAttachments: function (curProcess) {
        var postEvt = function () {
            var data = $("body").data(ARCD.AttData);
            if (data != null && data != undefined && data != "") {
                if (CONST.transaction_type.search == curProcess) {
                    ...
                    $(ARCD.Attachments).each(function () {
        ...
        if (CONST.transaction_type.save == curProcess) {
            $("#formAttachments .attachmentItem").each(function () {
                ...
                paramAttachments.status = ARCD.IsFinalize == true ? CONST.record_status.validated : $(this).attr("status");
        ...
        if (hasData == true) {
            Site.PostData("/DTS/MaintainDocumentAttachments", postEvt, param, ARCD.AttData);
        }
    },
    MaintainLogs: function (curProcess, curRemarks, postFinalEvt) {
        var postEvt = function () {
            ...
            if (curProcess == CONST.transaction_type.save && ARCD.IsFinalize == false) {
                Site.Dialogs.Alert(["Remarks successfully added to document with batch number ", ARCD.BatchId, "!"].join(''), "Got It!", null);
            }
        };
        var hasData = false;
        var param = ARCD.GetParameters(curProcess);
        param.documentLog.process = curProcess;
        param.documentLog.batch_id = ARCD.BatchId;
        if (CONST.transaction_type.save == curProcess) {
            var paramLogs = DTS.Clone(DTS.param.documentLog);
            paramLogs.batch_id = ARCD.BatchId;
        ...
        if (hasData == true) {
            Site.PostData("/DTS/MaintainDocumentLogs", postEvt, param, ARCD.AttData);
        }
    },
}
$(function () {
