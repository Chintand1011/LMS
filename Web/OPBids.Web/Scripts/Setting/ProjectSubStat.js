var PROJSUBSTAT = {
    ...
    Initialize: function () {
        if ($("#headerTitle").length <= 0) {
            setTimeout(PROJSUBSTAT.Initialize, 1000);
            return;
        }
        ...
        $(".toolbar-add").click(function (e) {
            ...
            PROJSUBSTAT.Status = CONST.record_status.activate;
        ...
        $(".toolbar-delete").click(function () {
            var confirmEvt = function () {
                ...
                PROJSUBSTAT.MaintainData(CONST.transaction_type.statusUpdate, 0, null, null, CONST.record_status.delete, _item_list);
            }
        ...
        $(".toolbar-search").click(function () {
            PROJSUBSTAT.MaintainData(CONST.transaction_type.search, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), PROJSUBSTAT.Status, null);
        });
        ...
        $("#btnSave").click(function () {
            if (Site.ValidateRequiredEntries("#saveModal", null) == true) {
                PROJSUBSTAT.MaintainData(CONST.transaction_type.save, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), PROJSUBSTAT.Status, null);
        ...
        $(".item-setting-edit").click(function (e) {
            ...
            PROJSUBSTAT.Status = $(["tr#row-" + _id, " td[status]"].join('')).attr("statusid").trim();
            $('#saveModal').modal('show');
            $("#saveModal *[required]").keyup();
        });
        ...
        $(".item-setting-inactive").click(function (e) {
            ...
            PROJSUBSTAT.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(),
        ...
        $(".item-setting-delete").click(function (e) {
            ...
            var confirmEvt = function () {
                ...
                PROJSUBSTAT.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), CONST.record_status.delete, _item_list);
            }
            Site.Dialogs.Confirm(["Remove ", $("#headerTitle").html()].join(''), ["Are you sure you want to remove selected ",
                $("#headerTitle").html(), "(s)?"].join(''), "Yes", "Cancel", confirmEvt, null);
        });
    },
    MaintainData: function (process, id, code, proj_desc, status, item_list) {
        var _result;
        var param = {
            'id': id,
            'proj_substatcode': code,
            'proj_substatdescription': proj_desc,
            'status': status,
        };
