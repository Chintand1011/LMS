var PROJCAT = {
    ...
    Initialize: function () {
        if ($("#headerTitle").length <= 0) {
            setTimeout(PROJCAT.Initialize, 1000);
            return;
        }
        ...
        $(".toolbar-add").click(function (e) {
            ...
            PROJCAT.Status = CONST.record_status.activate;
        ...
        $(".toolbar-delete").click(function () {
            var confirmEvt = function () {
                ...
                PROJCAT.MaintainData(CONST.transaction_type.statusUpdate, 0, null, null, CONST.record_status.delete, _item_list);
            }
        ...
        $(".toolbar-search").click(function () {
            PROJCAT.MaintainData(CONST.transaction_type.search, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), PROJCAT.Status, null);
        });
        ...
        $("#btnSave").click(function () {
            if (Site.ValidateRequiredEntries("#saveModal", null) == true) {
                PROJCAT.MaintainData(CONST.transaction_type.save, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), PROJCAT.Status, null);
        ...
        $(".item-setting-edit").click(function (e) {
            ...
            $("#u_id").val(_id);
            $("#u_code").val($(["tr#row-" + _id, " td[code]"].join('')).text().trim());
            $("#u_description").val($(["tr#row-" + _id, " td[description]"].join('')).html().trim());
            PROJCAT.Status = $(["tr#row-" + _id, " td[status]"].join('')).attr("statusid").trim();
            $('#saveModal').modal('show');
            $("#saveModal *[required]").keyup();
        });
        ...
        $(".item-setting-inactive").click(function (e) {
            ...
            PROJCAT.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(),
        ...
        $(".item-setting-delete").click(function (e) {
            ...
            var confirmEvt = function () {
                ...
                PROJCAT.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), CONST.record_status.delete, _item_list);
            }
            Site.Dialogs.Confirm(["Remove ", $("#headerTitle").html()].join(''), ["Are you sure you want to remove selected ",
                $("#headerTitle").html(), "(s)?"].join(''), "Yes", "Cancel", confirmEvt, null);
        });
