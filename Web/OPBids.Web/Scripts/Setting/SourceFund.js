var SRCFND = {
    ...
    Initialize: function () {
        if ($("#headerTitle").length <= 0) {
            setTimeout(SRCFND.Initialize, 1000);
            return;
        }
        ...
        $(".toolbar-add").click(function (e) {
            ...
            SRCFND.Status = CONST.record_status.activate;
        ...
        $(".toolbar-delete").click(function () {
            ...
            var confirmEvt = function () {                
                SRCFND.MaintainData(CONST.transaction_type.statusUpdate, 0, null, null, CONST.record_status.delete, _item_list);
            }
        ...
        $(".toolbar-search").click(function () {
            SRCFND.MaintainData(CONST.transaction_type.search, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), SRCFND.Status, null);
        });
        ...
        $("#btnSave").click(function () {
            if (Site.ValidateRequiredEntries("#saveModal", null) == true) {
                SRCFND.MaintainData(CONST.transaction_type.save, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), CONST.record_status.activate, null);
        ...
        $(".item-setting-edit").click(function (e) {
            ...
            SRCFND.Status = $(["tr#row-" + _id, " td[status]"].join('')).attr("statusid").trim();
            $('#saveModal').modal('show');
            $("#saveModal *[required]").keyup();
        });
        ...
        $(".item-setting-inactive").click(function (e) {
            ...
            SRCFND.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), CONST.record_status.deactivate, _item_list);
        });
        $(".item-setting-delete").click(function (e) {            
            ...
            var confirmEvt = function () {
                ...
                SRCFND.MaintainData(CONST.transaction_type.statusUpdate, $("#u_id").val(), $("#u_code").val(), $("#u_description").val(), CONST.record_status.delete, _item_list);
            }
            Site.Dialogs.Confirm(["Remove ", $("#headerTitle").html()].join(''), ["Are you sure you want to remove selected ",
                $("#headerTitle").html(), "(s)?"].join(''), "Yes", "Cancel", confirmEvt, null);
        });
    },
    MaintainData: function (process, id, source_code, source_description, status, item_list) {
        var _result;
        var param = {
            'id': id,
