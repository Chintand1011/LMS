...
(function ($, window, document) {
    ...
    function Wickedpicker(element, options) {
        ...
        this.AutoCloseTimePicker = function (ctl) {
            if ($(".wickedpicker").attr('focusPicker') == "false") {
                $(".wickedpicker").css({ "display": "none" });
            }
            else {
                var postEvt = function () {
                    ctl.AutoCloseTimePicker(ctl);
                }
                setTimeout(postEvt, 1000);
            }
        }
                ...
                if ($(self.timepicker).is(':visible')) {
                    ...
                    if ($(event.target).is(self.close)) {
                      self.hideTimepicker(window.lastTimePickerControl);
                        ...
                        $(".wickedpicker").unbind("mouseout").unbind("mouseover").mouseover(function () {
                            $(this).attr('focusPicker', true);
                        }).mouseout(function () {
                            $(this).attr('focusPicker', false);
                            var postEvt = function () {
                                self.AutoCloseTimePicker(self);
                            }
                            setTimeout(postEvt, 1000);
                            }).attr("focusPicker", true);
                        self.AutoCloseTimePicker(self);
