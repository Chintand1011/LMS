var APPROVED = {
    ...
    Initialize: function () {
        ...
        $(".toolbar-search").click(function () {
            ...
            APPROVED.MaintainData(CONST.transaction_type.search, null, null, null);
        });
    ...
    PopulateModal: function (_this) {
        ...
        APPROVED.GetAttachments(_id);
        APPROVED.GetProjectItems(_id);
    ...
    InitializeViewList: function () {
        var _result;
        ajaxHelper.Invoke("/Shared/GetSettingsList", JSON.stringify({ "setting_list": [CONST.setting_selection.ProjectCategory, CONST.setting_selection.ProjectGrantee] }), "", APPROVED.populateViewList(_result));
    },
            ...
            if (result) {
                ...
                $(result).each(function () {
                    ...
                    $("#formAttachments").append(["<div status ='", this.status, "'  attachmentid ='", this.id,
                        ...
                        this.attachment_name, "</div><a target='_blank' href='", FileUploader.GetPFMSDocument(APPROVED.UploadSubFolder + '/' + project_id + '/' + this.file_name),
                        "' title='Click to download' download><div class='", imgCls, "'></div></a><input type='text' placeholder='Barcode' maxlength='20' value='",
                        this.barcode_no, "' ", (APPROVED.EnableAttachments ? "" : "disabled"), "/></div>"].join(''));
                });
    ...
    GetProjectItems: function (project_id) {
        ...
        ajaxHelper.Invoke("/ProjectRequest/ProjectItems", _param, "json", function (result) {
            if (result) {
                ...
                $itemsTable.find("input.quantity, input.unit-cost").on("change", function () { APPROVED.ComputeItemTotalCost(this); });
