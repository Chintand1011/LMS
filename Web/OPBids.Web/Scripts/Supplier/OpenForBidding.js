var OFB = {
    ...
    GetParameters: function (cProc) {
        ...
        param.supplier.id = OFB.ProjectId;
        param.supplier.created_by = Site.UserId;
        param.supplier.updated_by = Site.UserId;
        param.documentAttachment.project_id = OFB.ProjectId;
        return param;
    },
    ...
    Initialize: function () {
        Site.Initialize();
        OFB.CurrentData = (new Date().getTime()).toString();
        OFB.AttData = [OFB.CurrentData, "-1"].join('');
        $("#pageContainer").remove();
        $("#formAttachments").unbind("filedrop");
        $("#formAttachments").filedrop({
            callback: function (blobData, curId, pFile) {
                OFB.ValidateAttachment(blobData, pFile[0].name);
            }
        });
        ...
        $("#btnSave").click(function () {
            ...
            var yesEvt = function () {
                OFB.MaintainData(CONST.transaction_type.save, CONST.project_substatus.closed_shortlist);
            }
            var noEvt = function () {
        ...
        $("#btnSearch").click(function () {
            OFB.SearchParam = OFB.GetParameters(CONST.transaction_type.search, null);
            OFB.MaintainData(CONST.transaction_type.search, null);
        });
        $("#fleBrowse").change(function () {
            OFB.ValidateAttachment(this.files[0], $(this).val());
        });
    ...
    MaintainData: function (currentProcess, cStat) {
        var param = OFB.GetParameters(currentProcess);
        if (cStat != null && cStat != "") {
            param.supplier.status = cStat;
        }
        var postEvt = function () {
            var data = $("body").data(OFB.CurrentData);
            $("#grdLst tbody").html('');
            ...
            if (data.value != null && data.value != undefined && data.value != "") {
                $(data.value).each(function () {
                    $("#grdLst tbody").append(["<tr itemid='", this.id, "'><td number>", this.index, "</td><td number>", this.ref_no,
                        ...
                        "</td><td project>", this.project, "</td><td amount class='money text-right' style='color:#0bce09'>", this.amount, "</td><td status>", this.status,
                        "</td><td>", Supplier.ContextMenu, "</td><td approved_budget style='display:none;' class='money'>",
                ...
                if (currentProcess == CONST.transaction_type.save) {
                    ...
                    OFB.MaintainAttachments(CONST.transaction_type.save);
                }
                $("#grdLst tbody tr").each(function () {
                    ...
                    $(this).find(".contextItemContainer").html([
                        ...
                        "<li><a class='dropdown-item item-setting-inactive lnkEdit' href='#' itemid='", $(this).attr("itemid"), "'>Bid on Project</a></li>",
                    ].join(''));
                ...
                $("#grdLst tbody tr .lnkEdit").click(function () {
                    OFB.EnableEdit(true);
                    OFB.PopUpModal($(this).attr("itemid"));
                ...
                $("#grdLst tbody tr .lnkView").click(function () {
                    OFB.EnableEdit(false);
                    OFB.PopUpModal($(this).attr("itemid"));
                    $("#btnRemarks").css({ "display": "inline-block" });
                    $("#btnFinalize").css({ "display": "none" });
                });
                
                //OFB.SetPagination(parseInt($(".paging").val()), data.total_count);
        ...
        Site.PostData("/Supplier/MaintainSupplier", postEvt, param, OFB.CurrentData);
    },
    EnableEdit: function (opt) {
        OFB.IsEnableEdit = opt;
        if (opt == true) {
    ...
    PopUpModal: function (itemId) {
        OFB.ProjectId = itemId;
        OFB.CurrentProcess = CONST.transaction_type.update;
        $("#u_project_description").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[project_desc]"].join('')).html());
        $("#u_approved_budget").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[approved_budget]"].join('')).html());
        $("#u_status").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[status]"].join('')).html());
        $("#u_category").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[category]"].join('')).html());
        $("#u_deadline").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[deadline]"].join('')).html());
        $("#u_bid_amount").val($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[amount]"].join('')).html());
        $("#u_project_duration").val($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[project_duration]"].join('')).html());
        $("#u_bid_bond").val($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[bid_bond]"].join('')).html());
        $("#generalTab").click();
        $(".modal-title").html($(["#grdLst tbody tr[itemid='", OFB.ProjectId, "'] td[document_code]"].join('')).html());
        $("#dataEntryModal input").keyup();
        $('#dataEntryModal').modal('show');
        OFB.MaintainAttachments(CONST.transaction_type.search);
        OFB.GetProjectItems(itemId);
    },
    MaintainAttachments: function (curProcess) {
        var postEvt = function () {
            var data = $("body").data(OFB.AttData);
                ...
                if (CONST.transaction_type.search == curProcess) {
                    $(data).each(function () {
                        ...
                        $("#formAttachments").append(["<div status ='", this.status, "' attachmentid ='", this.id,
                            ...
                            this.barcode_no, "' ", (this.status == CONST.record_status.validated || OFB.IsEnableEdit == false ? "disabled='disabled'" : ""),
                            " />", (this.status == CONST.record_status.validated || OFB.IsEnableEdit == false ? "" :
        ...
        var hasData = false;
        var param = OFB.GetParameters(curProcess);
        ...
        if (CONST.transaction_type.save == curProcess) {
            $("#formAttachments .attachmentItem").each(function () {
                ...
                paramAttachments.project_id = OFB.ProjectId;
        ...
        if (hasData == true) {
            Site.PostData("/Supplier/MaintainProjectAttachments", postEvt, param, OFB.AttData);
        }
    },
