...
{
    public class OpenForBiddingLogic : ApiController
    {
        ...
        public Result<IEnumerable<SuppliersVM>> GetData(SupplierPayloadVM payload)
        {
            ...
            predicate = predicate.And(p => p.project_substatus == Constant.ProjectRequest.ProjectSubStatus.OpenForBidding);
            ...
            var query = (from proj in db.ProjectRequests.Where(predicate)
                        ...
                        //where proj.record_status == Constant.RecordStatus.Active && //proj.project_substatus == ProjectSubStatus.OpenForBidding_SUPPLIER && 
            ...
            _result.total_count = query.Count();// _result.value.Count();
