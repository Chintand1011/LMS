...
{
    public class LostOpportunitiesLogic : ApiController
    {
        ...
        public Result<IEnumerable<SuppliersVM>> GetData(SupplierPayloadVM payload)
        {
            ...
            predicate = predicate.And(p => p.record_status == Constant.RecordStatus.Cancelled);
            ...
            var query = (from proj in db.ProjectRequests.Where(predicate)
                         join creator in db.AccessUser on proj.created_by equals creator.id
                             ...
                             //where //proj.record_status == Constant.RecordStatus.Cancelled &&
                             ...
                             select new 
                             {
                                 ...
                                 created_date = proj.created_date.ToString(),
                                 id = proj.id,
