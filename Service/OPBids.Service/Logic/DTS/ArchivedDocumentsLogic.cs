...
{
    public class ArchivedDocumentsLogic : ApiController
    {
        ...
        public Result<IEnumerable<DocumentsVM>> GetData(DocumentsPayload payload)
        {
            ...
            _result.value = (from d in db.ArchivedDocuments
                             ...
                             (s2.user_id == payload.filter.created_by || payload.filter.record_section == true ||
                             ((ds2.id == 1 || ds2.id == 2) &&
                             ...
                             select new DocumentsVM()
                             {
                                 ...
                                 record_category = d.record_category,
                             }).ToList();
        ...
        public Result<IEnumerable<DocumentsVM>> Update([FromBody] DocumentsPayload param)
        {
            ...
            try
            {
                using (var db = new DatabaseContext())
                {
                    var itm = db.ArchivedDocuments.FirstOrDefault(a => a.batch_id == param.document.id);
                    if (itm != null)
                    {
                        ...
                        db.ArchivedDocuments.AddOrUpdate(itm);
                        db.SaveChanges();
        ...
        public Result<IEnumerable<DocumentsVM>> UpdateStatus([FromBody] DocumentsPayload param)
        {
            ...
            try
            {
                using (var db = new DatabaseContext())
                {
                    param.updated_date = DateTime.Now;
                    param.id = 0;
                    var item = db.ArchivedDocuments.FirstOrDefault(a => a.batch_id == param.document.id);
                    item.status = param.document.status;
                    db.ArchivedDocuments.AddOrUpdate(item);
                    db.SaveChanges();
                    return GetData(param);
                }
            }
